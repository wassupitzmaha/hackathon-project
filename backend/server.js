...
requestHandler.listen(port, () => {
  ...
  requestHandler.get("/api/v1/FrontEndDeveloper", async (req, res) => {
    try {
      const dbResponse = await db.query("SELECT * FROM tech_professions WHERE profession= 'Front-End Developer' ");
  ...
  requestHandler.get("/api/v1/BackEndDeveloper", async (req, res) => {
    try {
      const dbResponse = await db.query("SELECT * FROM tech_professions WHERE profession= 'Back-End Developer' ");
      const result = dbResponse.rows.map(row => ({
        ...row,
        skills: row.skills.split(',').map(skill => skill.trim())
      }));
  ...
  requestHandler.get("/api/v1/DevOpsEngineer", async (req, res) => {
    try {
      const dbResponse = await db.query("SELECT * FROM tech_professions WHERE profession= 'DevOps Engineer' ");
      const result = dbResponse.rows.map(row => ({
        ...row,
        skills: row.skills.split(',').map(skill => skill.trim())
      }));
